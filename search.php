...
$searchSql = "SELECT * FROM produits WHERE nom LIKE ? ";
$searchPattern = '%' . str_replace(' ', '%', $searchQuery) . '%';
$params = [$searchPattern];
$types = "s";

if ($selectedCategoryId) {
    $searchSql .= " AND (id_cat = ? OR id_cat IN (SELECT id_cat FROM categories WHERE id_catparent = ?))";
...
$searchSql .= " ORDER BY " . ($sortColumns[$sortOption] ?? 'datecreation DESC');
            ...
            <div class="promotions">
                ...
                <div class="product-grid">
                    <?php if ($searchResult->num_rows > 0): ?>
                        <?php while ($product = $searchResult->fetch_assoc()): ?>
